-- PostgREST/Supabase ordena argumentos do RPC pelo nome; a ordem alfabética coloca
-- p_novo_cs_profissional_id antes de p_novo_horario. A assinatura antiga (date, time, uuid)
-- não coincidia → "Could not find the function ... in the schema cache".

DROP FUNCTION IF EXISTS public.painel_reagendar_cs_agendamento (uuid, uuid, date, time, uuid);
