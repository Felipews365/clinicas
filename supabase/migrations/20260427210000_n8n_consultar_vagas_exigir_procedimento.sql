-- Flag opcional p_exigir_procedimento: quando true, sem procedimento resolvido e sem p_profissional_id,
-- devolve [] (evita listar todos os profissionais quando o agente esquece de passar procedimento/servico_id).

DROP FUNCTION IF EXISTS public.n8n_cs_consultar_vagas (uuid, date, uuid, uuid, text);
...
CREATE OR REPLACE FUNCTION public.n8n_cs_consultar_vagas (
  ...
  p_exigir_procedimento boolean DEFAULT false
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF coalesce(p_exigir_procedimento, false) AND v_proc IS NULL AND p_profissional_id IS NULL THEN
    RETURN '[]'::jsonb;
  END IF;
...
REVOKE ALL ON FUNCTION public.n8n_cs_consultar_vagas (uuid, date, uuid, uuid, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.n8n_cs_consultar_vagas (uuid, date, uuid, uuid, text, boolean) TO anon;
GRANT EXECUTE ON FUNCTION public.n8n_cs_consultar_vagas (uuid, date, uuid, uuid, text, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.n8n_cs_consultar_vagas (uuid, date, uuid, uuid, text, boolean) TO service_role;

COMMENT ON FUNCTION public.n8n_cs_consultar_vagas (uuid, date, uuid, uuid, text, boolean) IS
  'Vagas por dia. Com p_exigir_procedimento=true e sem p_profissional_id, exige procedimento resolvido (senão []).';
