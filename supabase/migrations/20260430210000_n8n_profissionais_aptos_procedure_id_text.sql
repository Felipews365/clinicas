-- p_clinic_procedure_id como text: PostgREST/n8n enviam "" quando o modelo só preenche aptos_nome.
-- Mantém compatibilidade com UUID em string.
-- (Motivação no fluxo n8n: jsonBody não pode usar IIFE que apaga "{aptos_uuid}" antes da validação de placeholders.)

DROP FUNCTION IF EXISTS public.n8n_cs_profissionais_aptos_procedimento(uuid, uuid, text);
...
REVOKE ALL ON FUNCTION public.n8n_cs_profissionais_aptos_procedimento (uuid, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.n8n_cs_profissionais_aptos_procedimento (uuid, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.n8n_cs_profissionais_aptos_procedimento (uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.n8n_cs_profissionais_aptos_procedimento (uuid, text, text) TO service_role;

COMMENT ON FUNCTION public.n8n_cs_profissionais_aptos_procedimento (uuid, text, text) IS
  'Read-model: profissionais aptos ao procedimento. p_clinic_procedure_id como text (aceita vazio).';
