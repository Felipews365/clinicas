-- n8n_cs_consultar_vagas: p_profissional_id e p_clinic_procedure_id como text (aceita "" / inválido como NULL)
-- Alinha com n8n_cs_profissionais_aptos_procedimento e evita falhas PostgREST quando o modelo manda string vazia.

DROP FUNCTION IF EXISTS public.n8n_cs_consultar_vagas (uuid, date, uuid, uuid, text, boolean);
DROP FUNCTION IF EXISTS public.n8n_cs_consultar_vagas (uuid, date, uuid, uuid, text);
DROP FUNCTION IF EXISTS public.n8n_cs_consultar_vagas (uuid, date, uuid, text, text);
DROP FUNCTION IF EXISTS public.n8n_cs_consultar_vagas (uuid, date, uuid);
DROP FUNCTION IF EXISTS public.n8n_cs_consultar_vagas (uuid, date);
...
COMMENT ON FUNCTION public.n8n_cs_consultar_vagas (uuid, date, text, text, text) IS
  'Vagas por dia. Passe procedimento (UUID em text ou nome) para filtrar profissionais e calcular overlap pela duração do procedimento.';
