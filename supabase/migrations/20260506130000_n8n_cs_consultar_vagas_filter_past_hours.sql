-- n8n_cs_consultar_vagas: filtrar horários passados quando a data for hoje.
-- Problema: agente mostrava 08h, 09h, 10h, 11h às 12h (horários já passados).
-- Solução: excluir slots cujo horário < NOW() - 30min quando p_data = CURRENT_DATE.
-- Os 30 min de tolerância permitem ver o horário atual ainda como opção.
...
BEGIN
  ...
  -- Calcular corte de horário: só filtrar passados se for hoje
  IF p_data = CURRENT_DATE THEN
    v_cutoff_time := ((NOW() AT TIME ZONE 'America/Sao_Paulo') - interval '30 minutes')::time;
  ELSE
...
COMMENT ON FUNCTION public.n8n_cs_consultar_vagas (uuid, date, text, text, text) IS
  'Vagas por dia. Filtra horários passados (tolerância 30 min) quando data=hoje. Procedimento filtra aptidão; profissional_id inválido para o procedimento é ignorado.';
