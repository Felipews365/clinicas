-- Permite filtrar vagas pelo nome do procedimento (ex.: «extração») quando o agente não envia UUID.
-- Mantém p_clinic_procedure_id; se ambos forem informados, prevalece o UUID após validação.

DROP FUNCTION IF EXISTS public.n8n_cs_consultar_vagas (uuid, date, uuid, uuid);
...
COMMENT ON FUNCTION public.n8n_cs_consultar_vagas (uuid, date, uuid, uuid, text) IS
  'Vagas por dia; filtra por procedimento via UUID ou nome (p_procedimento_nome).';
