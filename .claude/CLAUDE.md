...
- **Normalização de telefone:** função `_phone_digits(text)` remove tudo que não é dígito — permite match entre `+5511999999999` e `5511999999999`
- **⚠️ `patients.phone` agora guarda SÓ dígitos** (migration `20260708180000_patients_phone_normalize_dedupe.sql`). Antes a constraint `UNIQUE (clinic_id, phone)` era sobre a string crua → `+5581…` e `5581…` eram tratados como pacientes diferentes e um batch de sync criou pares duplicados (a linha `+` com os agendamentos, a só-dígitos vazia). A migration: (1) dedupe geral por telefone normalizado (keeper = mais filhos, desempate mais antigo; repointa `appointments`/`cs_prontuario_*` antes de apagar os losers — `appointments` tem FK `ON DELETE RESTRICT`); (2) normaliza todos os `phone` remanescentes; (3) trigger **BEFORE INSERT/UPDATE `trg_patients_normalize_phone`** (`nullif(regexp_replace(phone,'\D','','g'),'')`) que mantém só dígitos em qualquer escrita futura (painel, sync, n8n) — assim `UNIQUE (clinic_id, phone)` volta a impedir dupes. Diagnóstico: `SELECT clinic_id, regexp_replace(phone,'\D','','g') pn, count(*) FROM patients GROUP BY 1,2 HAVING count(*)>1`
- **⚠️ `cs_clientes.telefone` deve conter APENAS dígitos** — nunca salvar o JID completo (`558196454656@s.whatsapp.net`). O `Create Cliente`/`Get Cliente` no n8n já usam `String(numCliente).split('@')[0].replace(/\D/g,'')`. Salvar o JID inteiro criava registros duplicados no painel pois a constraint unique `cs_clientes_clinic_telefone_uniq (clinic_id, telefone)` é por string exacta. **Defesa em BD (migration `20260708190000_cs_clientes_telefone_normalize_dedupe.sql`):** trigger **BEFORE INSERT/UPDATE `trg_cs_clientes_000_normalize_telefone`** (prefixo `000` p/ correr antes dos outros BEFORE triggers) faz `telefone := regexp_replace(telefone,'\D','','g')` — tira `@s.whatsapp.net` e qualquer não-dígito em qualquer escrita (painel, sync, n8n), logo o unique volta a impedir dupes. A migration também deduplicou o existente (keeper = nome confirmado → mais filhos → mais antigo; repointa `cs_agendamentos`/`crm_interacoes`/`crm_followup_tasks`/`cs_lembretes_enviados`/`cs_cliente_tags` antes de apagar; `cs_agendamentos` tem FK `ON DELETE RESTRICT`). Diagnóstico: `SELECT id, telefone FROM cs_clientes WHERE telefone LIKE '%@%'` ou `... WHERE telefone ~ '\D'`
...
- **Três modos de pausa (`whatsapp_sessions.pause_mode`, migration `20260527210000_pause_mode_rolling.sql`):**
  ...
  - **`rolling`** — modo legado do handoff (sessões antigas); `pause_until = now + 10 min` renovável; cada mensagem (staff **ou** cliente) renova a janela. Bot só volta após 10 min de **silêncio total**. **Novas** tomadas de conta do staff já não usam `rolling` (ver a seguir) — mas o RPC de reativação continua a tratá-lo para as sessões que ainda o têm.
...
- **Pausa do handoff configurável por clínica (migration `20260708200000_handoff_pausa_configuravel.sql`):** a clínica escolhe **por quanto tempo a IA fica pausada quando o staff assume pelo WhatsApp**. Guardado em `clinics.agent_instructions->>'handoff_pausa_minutos'` (texto→JSON): `> 0` = minutos de pausa `timed`; `0` = `manual` (só volta ao reativar no painel); ausente = default **60 min**. Helper `public._clinic_handoff_pausa_minutos(uuid)` (espelha `_clinic_antecedencia_minutos`).
  - **`n8n_cs_staff_assumir_sessao` reescrita:** lê o helper e faz `bot_ativo=false` + upsert da sessão com `pause_mode = 'timed'` (ou `'manual'` se `<=0`) e `pause_until = now + minutos`. **Cada mensagem do staff reinicia a janela** (o staff continua ativo); mensagens do cliente **não** estendem `timed`, portanto a IA volta exactamente após o tempo configurado sem novas mensagens do staff — resolve a reclamação «a IA continua a responder depois de a clínica assumir». Pausa `manual` vinda do painel tem prioridade e nunca é rebaixada.
  - **UI:** seletor «Quando eu respondo pelo WhatsApp, pausar a IA por…» no topo da tela **Clientes** (`painel-clientes-cs.tsx`), grava `handoff_pausa_minutos` fazendo merge no JSON de `agent_instructions`. Opções 15 min … 24 h + «Até eu reativar» (`0`).
...
- **Painel web (`painel-clientes-cs.tsx`)** — redesenhado com **duas abas:**
  ...
  - **Quem falou por último (2026-07-08):** `loadRows` também lê `n8n_chat_histories` (`session_id LIKE clinic_id:%`, `order id desc`, limit 2000) e reduz à última mensagem por telefone (`lastMsgs: LastMsgMap`). Cada card mostra uma pílula no início — **Cliente** (âmbar, `message.type='human'` → aguarda resposta) ou **Agente** (azul, `type='ai'`) — mais um preview do texto. ⚠️ Mensagens do staff pelo próprio WhatsApp **não** entram em `n8n_chat_histories`, logo se o staff respondeu pelo celular o último registo ainda é `human` e a pílula continua «Cliente» (mesma limitação do inbox).
...
### Prontuário eletrônico (`web/src/components/prontuario/prontuario-panel.tsx`)
Módulo do painel isolado por clínica (RLS `rls_has_clinic_access(clinic_id)`), ligado a `patients` (fonte de verdade do painel). Item «Prontuário» na sidebar. Tabelas: `cs_prontuario_ficha` (1 linha/paciente), `cs_prontuario_registros` (linha do tempo), `cs_prontuario_anexos` (bucket **privado** `prontuario`, 10 MB, PDF/JPG/PNG; path `{clinic_id}/{patient_id}/arquivo`).

- **Fase 1** (migration `20260708160000_prontuario_eletronico.sql`): abas **Ficha** (upsert de alergias/crônicas/medicações/tipo sanguíneo/observações), **Evoluções** (criar/apagar registos) e **Anexos** (upload/abrir via signed URL 120 s/apagar).
- **Fase 2** (migration `20260708170000_prontuario_documentos_tipos.sql` — expande o CHECK de `cs_prontuario_registros.tipo` para `receita`/`declaracao` além de `anamnese`/`evolucao`/`atestado`/`nota`):
  - **Editar evoluções** inline (tipo/título/conteúdo/profissional/consulta) via `update ... where clinic_id + id`.
  - **Ligar evolução à consulta** — dropdown de `appointments` do paciente grava `cs_prontuario_registros.appointment_id`; o registo mostra 📅 data/serviço da consulta.
  - **Aba Documentos** — filtra registos por `tipo in (atestado, receita, declaracao)`; a aba Evoluções filtra `tipo in (evolucao, anamnese, nota)`. Form com modelo pré-preenchido por tipo (`docTemplate`) + profissional para assinatura.
  - **Imprimir/PDF sem lib externa** — `openPrint()` abre nova janela com HTML+CSS inline e chama `window.print()` (a janela nova escapa à CSP do app). Botão «Imprimir prontuário» no cabeçalho gera ficha + linha do tempo cronológica; botão 🖨 por documento gera atestado/receita/declaração com cabeçalho da clínica + assinatura do profissional. Conteúdo escapado com `esc()`/`nl2br()`.
  - **Cabeçalho da clínica** lido de `clinics` (`name`, `phone`, `endereco` parseado de `agent_instructions` que é **text** com JSON, não jsonb). `professionals.specialty` vai na linha de assinatura.
- **Tenant-safe:** todas as queries filtram `clinic_id`; RLS garante isolamento; anexos no bucket privado com policies por `split_part(name,'/',1)::uuid`.
