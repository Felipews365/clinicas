...
- **Trigger `trg_sync_professional_to_cs`** (migration `20260504330000`) sincroniza `professionals` → `cs_profissionais` em INSERT, UPDATE **e DELETE**:
...
- **Fluxo busca de horários — 2 etapas obrigatórias (actualizado 2026-05-06):**
  - **Etapa 2A:** com o procedimento confirmado, o agente lê o **`## MAPA PROCEDIMENTO → PROFISSIONAIS APTOS`** injectado pelo `Enrich Agendador` e mostra **apenas os nomes** dos profissionais aptos — **sem chamar nenhuma tool**, sem mostrar horários. Pergunta: «Com qual profissional prefere?»
...
- **`Enrich Agendador`** injeta no contexto o bloco **`## MAPA PROCEDIMENTO → PROFISSIONAIS APTOS`**: mapa calculado em runtime (serviços + `professional_procedures` do banco) mostrando para cada procedimento quais profissionais têm vínculo explícito. Profissionais sem nenhum vínculo em `professional_procedures` **não aparecem** neste mapa nem nos resultados da RPC quando um procedimento é especificado — precisam ter os procedimentos configurados no painel para aparecer em buscas.
...
- **Painel toggle pausar = clínica assumir via WhatsApp — são equivalentes:** ambos fazem `bot_ativo=false` + `staff_handling=true` + `updated_at=now()` e ativam o timer de 10 min de reativação automática. O resultado é idêntico para o n8n.
...
- **Auto-reativação após 10 min de inatividade do staff (migration `20260506120000`):** quando o staff assume a conversa (via WhatsApp ou painel), `whatsapp_sessions.updated_at` é carimbado com o instante da última mensagem do staff. O n8n verifica isso no node **`HTTP Verifica Inatividade 10min`** (TRUE branch de "Bot inativo") → **`IF Bot Reativado`** (TRUE → "Verifica se é Image, Texto ou Áudio1" = fluxo normal retoma). A RPC `n8n_cs_verificar_reativar_bot` usa `whatsapp_sessions.updated_at` + `staff_handling = true` como critério — **não** `n8n_chat_histories` (bug anterior: contava a partir da última resposta do agente, não do staff). Regras: só reativa se `staff_handling = true` E `updated_at < now() - 10 min`; se `needs_human = true` (aguardando staff que ainda não respondeu) → não reativa.
...
- **Painel web (`painel-clientes-cs.tsx`)** — redesenhado com **duas abas:**
  ...
  - Toggle ao **pausar**: `UPDATE cs_clientes SET bot_ativo = false` + upsert `whatsapp_sessions` com `staff_handling = true, updated_at = now()` → activa o timer de 10 min. Toggle ao **reativar manualmente**: `bot_ativo = true` + limpa `staff_handling = false` na sessão. Realtime subscription para atualização automática.
  - **Status efectivo no painel (`botEffective`)**: o toggle mostra ATIVO mesmo com `bot_ativo = false` no banco se `staff_handling = true` E `whatsapp_sessions.updated_at` passou de 10 min — reflecte que o n8n reativaria na próxima mensagem. Tick de 60 s força re-render sem evento do banco.
  - **Countdown "reativa em X min"**: abaixo do label "PAUSADO", mostra o tempo restante até reactivação automática com base em `whatsapp_sessions.updated_at`. O timer reinicia a cada mensagem do **staff** (fromMe=true → `n8n_cs_staff_assumir_sessao` atualiza `updated_at`); mensagens do **cliente** durante pausa **não** reiniciam o timer. Realtime de `whatsapp_sessions` atualiza o countdown em tempo real.
