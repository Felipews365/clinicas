/**
 * patch-fix-monta-contexto-v3.mjs
 *
 * Corrige a referência do node no "Monta Contexto":
 *   ERRADO:  $('Buscar Config Clínica')  — o í (U+00ED) não bate com o nome real do node
 *   CORRETO: $('Buscar Config Cl?nica')  — o node está salvo com ? literal (encoding corrompido)
 *
 * Sem essa correção o try/catch silencia o erro e clinicRaw = {},
 * deixando nome_agente, clinic_name, agent_instructions etc. vazios.
 */
...
function patchNodes(nodeArr) {
  ...
  for (const node of nodeArr) {
    if (node.name === 'Monta Contexto' && node.type === 'n8n-nodes-base.code') {
      const before = (node.parameters?.jsCode || '').split('\n')[0];
      node.parameters = { ...node.parameters, jsCode: CORRECT_CODE };
      const after = (node.parameters?.jsCode || '').split('\n')[0];
