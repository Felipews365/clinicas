/**
 * Corrige o Code auto-notify profissional para parsear resposta da RPC
 * de forma robusta: $helpers.httpRequest pode retornar string, array ou objeto.
 */
import fs from "fs";
...
// Atualiza em nodes[] top-level
const n = wf.nodes.find((x) => x.name === "Code auto-notify profissional");
if (!n) throw new Error("Code auto-notify profissional not found in nodes[]");
n.parameters.jsCode = newCode;

// Atualiza em activeVersion.nodes[] se existir
const av = wf.activeVersion?.nodes?.find((x) => x.name === "Code auto-notify profissional");
if (av) av.parameters.jsCode = newCode;

fs.writeFileSync(wfPath, JSON.stringify(wf, null, 2) + "\n");
console.log("OK: local JSON updated");
